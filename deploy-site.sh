...
if [[ $WEBSERVER == apache2 ]]
then
  sudo printf "<VirtualHost *:80>\n\tServerName $DOMAIN\n\tRedirect 301 /.well-known/acme-challenge http://$CMS/.well-known/acme-challenge\n\tDocumentRoot /var/www/html\n\tRewriteEngine on\n\tRewriteCond %%{REQUEST_URI} !^/.well-known/acme-challenge\n\tRewriteRule ^ https://%%{SERVER_NAME}%%{REQUEST_URI} [END,NE,R=permanent]\n</VirtualHost>\n" > $CONFIGPATH$DOMAIN$DEFAULT
elif [[ $WEBSERVER == nginx ]]
then
  sudo printf "server {\n\tlisten 80;\n\tlisten [::]:80;\n\tserver_name $DOMAIN;\n\troot /usr/share/nginx/html;\n\trewrite ^/.well-known/acme-challenge http://$CMS\$request_uri permanent;\n\treturn 301 https://\$host\$request_uri;\n}\n" > $CONFIGPATH$DOMAIN$DEFAULT
fi
...
then
  ...
  then
    ...
    printf "Gracefully reloading the $WEBSERVER service...\n"
  ...
  printf "After all HTTP sites have been created, run the deploy-cert.sh script from your management server before continuing.\n\n"
...
then
  ...
  then
    ...
    printf "Gracefuly reloading the $WEBSERVER service...\n"
    sudo apachectl -k graceful
  elif [[ $WEBSERVER == nginx ]]
  then
